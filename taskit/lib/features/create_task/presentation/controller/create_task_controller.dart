...
class CreateTaskController extends Notifier<CreateTaskState>{
  ...
  void mapToSubtaskReq(List<TextEditingController> subtaskControllers) {
    state = state.copyWith(
      subtasks: subtaskControllers.where((controller)=>controller.text.isNotEmpty).map((e) => SubtaskReq(title: e.text)).toList(),
    );
  }
  ...
  Future<void> saveTask() async {
    ...
    print(
        state.createTaskForm['title'].toString()+
    ...
    try {
      ...
      final form = CreateTaskReq(
        ...
        dueDate: state.selectedDate ?? DateTime(
            DateTime
                .now()
                .year,
            DateTime
      ...
      final result = await ref.read(taskServiceProvider).createTask(
          token!, form);
      result.when((success) {
