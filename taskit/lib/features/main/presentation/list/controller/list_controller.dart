...
class ListController extends Notifier<ListState> {
  ...
  Future<void> updateTasks() async {
    try {
      ...
      final result = await ref.read(taskServiceProvider).getListTask(
          token!, state.selectedStatus!, state.selectedDate!.toIso8601String());
      result.when((success) {
  ...
  Future<void> onFirstUpdateTasks() async {
    try {
      ...
      final result = await ref
          ...
          .getListTask(token!, 'pending', DateTime.now().toIso8601String());
      result.when((success) {
        state = state.copyWith(tasks: success);
      }, (failure) {
        state = state.copyWith(error: failure.message);
    ...
    try {
      ...
      result.when((success) {
        state = state.copyWith(
            tasks: state.tasks
                .map((e) => e.id == taskId ? e.copyWith(status: status) : e)
                .toList());
      }, (failure) {
        state = state.copyWith(error: failure.message);
      });
