...
import '../../data/dtos/request/forgot_pass_verify/forgot_pass_verify_req.dart';
import '../../data/dtos/request/reset_pass/reset_pass_req.dart';

final forgotPassControllerProvider =
  NotifierProvider<ForgotPassController,ForgotPassState>(ForgotPassController.new);
...
class ForgotPassController extends Notifier<ForgotPassState>{
  ...
  Future<void> forgotPass() async {
    try {
      ...
      final formData = ForgotPassRequest(
        email: state.forgotPassForm['email']
      );
      final result = await ref.read(forgotPassServiceProvider).forgotPass(formData);
      result.when((success) {
        state = state.copyWith(
          isLoading: false,
  ...
  Future<void> reset() async {
    try {
      ...
      final result = await ref.read(forgotPassServiceProvider).reset(formData,state.forgotPassVerifyModel!.resetToken);
