...
class AIServices {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a smart assistant. Please answer only in JSON format without any special characters or additional text",
            },
            {
              role: "user",
              content: `Give me a list of 5 suitable categories for the task with title: ${title}. Answer only in JSON format, where the response is in this format:["category1", "category2", "category3", "category4", "category5"] and each of responses only contain maximum two words. In addition, 3 of the categories must be one word only, other can be two words. Also, exclude the following categories: ${excludedCategories}.Upcase each first letter. No special symbols, no extra text.`,
            },
          ],
        },
        {
  ...
  static async createTaskUsingAi(
    userInput,
    categoryList = [],
    userUtcOffset = "+00:00",
  ) {
...
Respond with valid pure JSON only. No explanation, no notes.`;
      ...
      if (!choices || choices.length === 0) {
        throw new ServerError(
          "AI did not return any choices, please  resend your request.",
          503,
        );
      }
      ...
      if (!rawResult) {
        throw new ServerError("AI response is missing content.");
      }
  ...
  static async answerTaskQuestion(
    userInput,
    categories = [],
    tasks = [],
    subtasks = [],
    ...
    try {
      ...
      if (!answer) {
        throw new ServerError("No response from AI. Try again.");
      }
